	...
	<v-sheet>
		...
		<v-sheet class="d-flex flex-column justify-center align-center text-none">
			...
			<v-radio-group inline>
				<v-radio v-for="color in product.colors" :key="color.id" :label="color.name" :value="color.value" color="#cd3545">
				</v-radio>
			</v-radio-group>
		</v-sheet>
		...
		<v-sheet rounded="lg" class="my-3 pa-2 border border-success">
		...
		<v-sheet class="action d-flex mt-2 w-100">
			<v-btn rounded="0" @click="cartStore.add(product.id)" href="" stacked color="#cd3545"
				class="text-white text-none text-caption w-75 rounded-s-lg">
				<h3 class="text-uppercase">Mua ngay</h3>
				<p class="text-small text-center px-1">Giao nhanh trong 2 giờ hoặc nhận tại cửa hàng</p>
			</v-btn>
			<v-btn rounded="0" color="#cd3545" variant="outlined" stacked class="px-1 text-none w-25 rounded-e-lg"
				@click="cartStore.add(product.id)">
				<v-icon>mdi-cart-plus</v-icon>
				<p class="text-small text-center">Thêm giỏ hàng</p>
			</v-btn>
		</v-sheet>

		<!-- Ưu đãi thêm -->
		<v-sheet rounded="lg" class="my-3 pa-2 text-medium text-justify" border="sm">
					...
					<tr>
						<th colspan="2">
							<div class="d-flex align-center">
								<v-icon color="#cd3545">mdi-product-outline</v-icon>
