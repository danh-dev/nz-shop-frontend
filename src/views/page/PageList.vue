...
async function deletePage(id) {
  try {
    await axios.delete(`http://127.0.0.1:8000/api/pages/delete/${id}`);
  } catch (error) {
    console.log("Error delete page: ", error);
  }
}
function editPage(name) {
  router.push(`page/edit/${getSlugByName(name)}`);
}
        ...
        <tr v-for="item in pages.slice((page - 1) * rowsPerPage, page * rowsPerPage)" :key="item.id">
          ...
          <td>
            <div class="d-flex align-center justify-space-between">
              <v-btn @click="editPage(getSlugByName(item.name))" size="small" variant="tonal" icon="mdi-text-box-edit-outline"
