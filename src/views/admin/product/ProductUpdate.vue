...
const submit = async () => {
  ...
  if (galleryArray.value.length > 0) {
    formData.append("newGallery", galleryArray.value.join("|"));
  }
...
const fetchOneProductById = async () => {
  try {
    const res = await axios.get(`products/id/${route.params.id}`);
    if (res.status === 200) {
      const product = mapKeys(res.data.data, (value, key) => camelCase(key));
      mainForm.value.category.value = product.categoryId;
      mainForm.value.name.value = product.name;

      mainForm.value.sku.value = product.sku;
      mainForm.value.quantity.value = product.quantity;

      mainForm.value.originPrice.value = product.originPrice;
      mainForm.value.sellPrice.value = product.sellPrice;
      if (product.discountPrice) {
        mainForm.value.discountPrice.value = product.discountPrice;
        mainForm.value.startDate.value = product.startDate;
      ...
      if (product.variants) {
        mainForm.value.multiVariant.value = true;
        const variantsTemp = JSON.parse(product.variants);
        for (const index in variantsTemp) {
          addOneMore();
          variants.value[index].name = variantsTemp[index].name;
          variants.value[index].quantity.value = variantsTemp[index].quantity;
          variants.value[index].originPrice.value = variantsTemp[index].originPrice;
          variants.value[index].sellPrice.value = variantsTemp[index].sellPrice;
          variants.value[index].startDate.value = variantsTemp[index].startDate;
          variants.value[index].endDate.value = variantsTemp[index].endDate;
        }
      }
    }
  }
  ...
  <v-container class="m-card my-3">
    ...
    >
      ...
      <v-row>
        ...
        <v-col
          v-if="mainForm.gallery.value.length || galleryArray.length"
          cols="12"
          class="d-flex flex-wrap"
        >
          <div class="d-flex flex-wrap">
            <v-img
              v-for="(path, index) in galleryArray"
              :key="index"
              :src="`${url}${path}`"
              :max-width="80"
              :min-width="80"
            ...
            >
              <v-btn
                ...
                variant="flat"
                @click=" galleryArray.splice(index, 1)"
              >
                <v-icon
                  icon="mdi-close"
                  size="15"
