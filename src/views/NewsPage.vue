...
<template>
  <v-sheet>
    ...
    <v-row>
      <v-col :cols="12" lg="8" md="8" v-if="articles.length > 0">
        <h4 class="py-2 text-danger">Tin tức cập nhật</h4>
        <v-sheet v-for="article in articles.slice((page - 1) * rowsPerPage, page * rowsPerPage)" :key="article.id"
    ...
    <GlobalPagination v-if="articles.length > rowsPerPage" :numberOfPages="numberOfPage" :page="page"
