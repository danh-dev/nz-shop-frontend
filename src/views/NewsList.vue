      ...
      <v-col cols="12">
        <v-sheet>
          <v-carousel cycle height="24rem" hide-delimiter-background show-arrows="hover">
            <v-carousel-item v-for="item in posts.slice(0,5)" :key="item.id"
              ...
              <v-sheet height="100%" class="border rounded-lg">
                <div class="d-flex fill-height justify-center align-center mx-4">
                  <img class="w-75 h-75 rounded-lg" :src="`${url + item.image}`" alt="" />
                  <div class="px-3 text-justify">
                    <a :href="`/news/${getSlugByName(item.title)}`" class="text-body font-weight-bold">{{ item.title
                    }}</a>
    ...
    <v-row>
      <v-col :cols="12" lg="8" md="8" v-if="posts.length > 0">
        ...
        <v-sheet v-for="item in posts.slice((page - 1) * rowsPerPage, page * rowsPerPage)" :key="item.id"
          ...
          <v-img class="w-50 me-2 rounded-lg" :src="`${url + item.image}`" />
      ...
      <v-col cols="12" lg="4" md="4">
        <v-sheet class="col-12">
          ...
          <div v-for="item in posts.slice(5, 10)" :key="item.id" class="my-2 w-75 px-2">
            <img :src="`${url + item.image}`" alt="" class="w-100 h-50 rounded-lg" />
            <div>
